          ...
          <section className="space-y-3 rounded-2xl border border-border bg-card p-4">
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <label className="block text-xs text-muted">
                Draft type
                <select className="mt-1 w-full rounded-lg border border-border px-3 py-2 text-sm" value={d.draftType} onChange={(e) => patch({ draftType: e.target.value, selection: e.target.value.includes("Salary") ? "linear" : d.selection })}>
                  {[
                    "Live Standard Draft",
            ...
            <Board
              title="Top 20 available (pre-draft)"
              rows={shownSetup}
        ...
        {rows.map((row, i) => (
          <li key={row.player.id} className="flex items-center justify-between gap-2 rounded-xl border border-border px-3 py-2">
            ...
            <div className="flex shrink-0 gap-1.5">
              <Button className="h-9 px-2 text-xs" onClick={() => onMine(row.player)}>
                My Pick
              </Button>
              <Button variant="secondary" className="h-9 px-2 text-xs" onClick={() => onTaken(row.player)}>
                Taken
              </Button>
            </div>
          </li>
