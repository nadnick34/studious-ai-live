...
export const getTeacherToolState = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    try {
      ...
      await sql.query(`alter table teacher_tool_state add column if not exists fantasy_draft jsonb`);
      await sql.query(`alter table teacher_tool_state add column if not exists fantasy_draft jsonb`);
    } catch {
      /* ignore */
    }
    const rows = await sql<{ scriptorium_log: unknown; testprep_cards: unknown; student_testprep: unknown; college_compare: unknown }>`
      select scriptorium_log, testprep_cards, student_testprep, college_compare, teacher_prep_tracks, fantasy_draft from teacher_tool_state where user_id = ${context.userId} limit 1
    `;
    const row = rows[0];
    return {
      ...
      fantasyDraft: row?.fantasy_draft ?? null,
    };
  });
...
export const saveTeacherToolState = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const fantasyDraft =
      data.fantasyDraft !== undefined ? data.fantasyDraft : (cur[0] as { fantasy_draft?: unknown })?.fantasy_draft ?? null;
    try {
      await sql.query(`alter table teacher_tool_state add column if not exists student_testprep jsonb not null default '[]'::jsonb`);
      await sql.query(`alter table teacher_tool_state add column if not exists college_compare jsonb`);
      await sql.query(`alter table teacher_tool_state add column if not exists teacher_prep_tracks jsonb not null default '[]'::jsonb`);
    ...
    await sql`
      insert into teacher_tool_state (user_id, scriptorium_log, testprep_cards, student_testprep, college_compare, teacher_prep_tracks, fantasy_draft, updated_at)
      values (${context.userId}, ${JSON.stringify(scriptoriumLog)}::jsonb, ${JSON.stringify(testPrepCards)}::jsonb, ${JSON.stringify(studentTestPrep)}::jsonb, ${JSON.stringify(collegeCompare)}::jsonb, ${JSON.stringify(teacherPrepTracks)}::jsonb, ${JSON.stringify(fantasyDraft)}::jsonb, now())
      on conflict (user_id) do update set
        ...
        fantasy_draft = excluded.fantasy_draft,
        updated_at = now()
    `;
    return { ok: true as const };
  });
